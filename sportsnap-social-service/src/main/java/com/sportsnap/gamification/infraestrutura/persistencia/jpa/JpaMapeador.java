...
class JpaMapeador {
    ...
    Perfil paraDominio(PerfilJpa jpa) {
        return new Perfil(new PerfilId(jpa.id), new AtletaId(jpa.usuarioId),
            ...
            jpa.totalSeguidores, jpa.totalSeguindo, jpa.fotoPerfil);
    }
    ...
    PerfilJpa paraJpa(Perfil dominio) {
        ...
        jpa.fotoPerfil      = dominio.getFotoPerfil();
        return jpa;
    }
    ...
    PostEsportivo paraDominio(PostEsportivoJpa jpa) {
        return new PostEsportivo(new PostEsportivoId(jpa.id),
            new PerfilId(jpa.autorId), jpa.conteudo, jpa.esporte, jpa.criadoEm);
    }

    PostEsportivoJpa paraJpa(PostEsportivo dominio) {
        var jpa = new PostEsportivoJpa();
        if (dominio.getId() != null) jpa.id = dominio.getId().getId();
        jpa.autorId   = dominio.getAutorId().getId();
    ...
    Comentario paraDominio(ComentarioJpa jpa) {
        var parentId = jpa.parentId != null ? new ComentarioId(jpa.parentId) : null;
        return new Comentario(new ComentarioId(jpa.id),
            new com.sportsnap.gamification.dominio.feed.ItemFeedId(jpa.itemFeedId),
            new com.sportsnap.gamification.dominio.perfil.PerfilId(jpa.autorId),
            jpa.conteudo, parentId, jpa.criadoEm);
    }

    ComentarioJpa paraJpa(Comentario dominio) {
        var jpa = new ComentarioJpa();
        if (dominio.getId() != null) jpa.id = dominio.getId().getId();
        jpa.itemFeedId = dominio.getItemFeedId().getId();
        jpa.autorId    = dominio.getAutorId().getId();
        jpa.conteudo   = dominio.getConteudo();
        jpa.parentId   = dominio.getParentId() != null ? dominio.getParentId().getId() : null;
        jpa.criadoEm   = dominio.getCriadoEm() != null ? dominio.getCriadoEm() : LocalDateTime.now();
        return jpa;
    }
