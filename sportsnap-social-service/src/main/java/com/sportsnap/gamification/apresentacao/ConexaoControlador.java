...
public class ConexaoControlador {
    ...
    public void desbloquear(@RequestBody ConexaoDto dto) {
        conexaoServico.desbloquear(new PerfilId(dto.seguidorId), new PerfilId(dto.seguidoId));
    }

    @GetMapping("/{perfilId}/bloqueados")
    public List<Bloqueio> listarBloqueados(@PathVariable int perfilId) {
        var id = new PerfilId(perfilId);
        return bloqueioRepositorio.listarEnvolvendo(id).stream()
            .filter(b -> b.getBloqueadorId().equals(id))
            .toList();
    }
